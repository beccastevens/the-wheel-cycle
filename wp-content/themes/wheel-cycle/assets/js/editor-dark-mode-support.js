/* global wheelcycleIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'wheelcycle-supports-dark-theme' ) ) {
	wheelcycleDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function wheelcycleDarkModeEditorInit( attempt ) {
	...
	if ( wheelcycleIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						wheelcycleDarkModeEditorInit( attempt + 1 );
					},
